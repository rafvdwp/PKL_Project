                    ...
                    <div class="mx-auto max-w-8xl px-4 sm:px-2">
                        <div class="grid grid-cols-1 gap-4">
                            <div class="mb-2 bg-gray-800 border-solid rounded-lg shadow-sm w-full">
                                <form action="{{ route('admin.project.managesubSystem.store') }}" method="post">
                                    @csrf
                                    <div
                                    ...
                                    <div class="p-3">
                                        <label class="block">
                                            ...
                                            <div class="flex">
                                                <input type="text" value="{{ old('kategori_brg') }}" name="name"
                                                    class="mt-1 px-4 py-1 bg-gray-700 border shadow-sm border-slate-300 placeholder-slate-400 block w-full rounded-lg text-white"
                                                    required />
                                                <button
                                                    class="ml-3 bg-gray-700 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-full">
                                                    Submit
                                                </button>
                                            </div>
                                        </label>
                            ...
                            <table class="w-full text-sm text-left rounded rtl:text-right text-gray-400">
                                ...
                                <tbody class="bg-gray-800">
                                    @foreach ($managesubSystem as $item)
                                        <tr class="odd:bg-gray-900 even:bg-gray-800 border-b border-gray-500">
                                            <th scope="row" class="px-6 py-4 font-bold whitespace-nowrap text-white">
                                                <a href="">{{ $item->name }}</a>
                                            </th>
                                            {{-- <td class="px-6 py-4 gap-2 justify-center">
                                                    
                                            </td> --}}
                                            <td class="px-6 py-2 flex gap-2 justify-center">
                                                <form
                                                    action="{{ route('admin.project.managesubSystem.delete', $item->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="font-medium text-white rounded p-1"
                                                        style="background-color:rgb(55 65 81);"><svg class="w-6 h-6"
